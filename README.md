...
The workflow `.github/workflows/check-news.yml` calls `/api/check` every 5 minutes, offset to minutes 2,7,12,… (`2-59/5 * * * *`) to avoid GitHub's most crowded schedule boundary. The endpoint itself lives on Vercel and sends Telegram messages.
